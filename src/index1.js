...
var StringArray = ArrayType(ref.types.char, 100);

//加载 DLL文件,无需写扩展名,将DLL中的函数映射成JS方法
const MyDellDemo = new ffi.Library(path.resolve("resources/MYDLLDEMO"), {
  //方法名必须与C函数名一致
  add: [
    "int", // 对应 C函数返回类型
    ["int", "int"] // C函数参数列表
  ...
  initArrayChat: ["long", [StringArray, StringArray]]
});
const IDReader = new ffi.Library(path.resolve("resources/terminal/IDReader"), {
  //方法名必须与C函数名一致
...
// }
// const bufferValue = Buffer.from("Hello World");
// const bufferValue2 = Buffer.from("");

// console.log('initChatArray',MyDellDemo.initArrayChat(stringArray,stringArray2))
// console.log('StringArray',stringArray)
var result1 = new Buffer(150);
console.log("IDReader", IDReader.iReadMultiCard("D:\\", result1));
console.log("result", result1.toString("utf-8", 0, result1.length));
